  ...
  data() {
    return {
      ...
      act_btn: [
        ...
        {name: '完成报价', code: 'OVERQUOTE', when: {'billState': [2]}, action: updateProduceQuote},
        {name: '查看',code: 'LOOKUP', when: {'billState': [0,1,5, 6]}, action: updateProduceQuote}
      ],
      replace: [
      ...
      this.$confirm('是否确认完成生产报价?', '提示', {
        ...
        sendDingMsgByPsndoc(sendMsgParam).then((response) => {
          if (!response.data.success) {
            ...
            let updateParam = { pkProduceQuoteH: bindRowData.pkProduceQuoteH, pkProduct: bindRowData.pkProduct,taskId: bindRowData.taskId }
