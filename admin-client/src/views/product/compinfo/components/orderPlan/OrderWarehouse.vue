<template>
    ...
    >
        ...
        <el-button
          size="mini"
          @click="handleEdit(scope.$index, scope.row.pkWarehouse)">下载</el-button>
        <el-button v-if="scope.row.onOffState==0"
          size="mini"
          @click="onOffState(scope.row.pkWarehouse,1)">暂停</el-button>
        <el-button
          size="mini" v-if="scope.row.onOffState==1"
          @click="onOffState(scope.row.pkWarehouse,0)">启动</el-button>          
      </template>
          </el-table-column>
          </el-table>
    <div class="fenye">
...
import {listWarehouse,updateWarehouse,updateWarehouseState} from "@/api/productMageMent/productMageMent";
